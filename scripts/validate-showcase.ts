...
import { generatedManifestPath, hashDirectory, normalizedJson, readAndValidateReport, resolveCanonicalArtifactPath } from './showcase-lib';
...
export function validateShowcasePackage() {
  ...
  for (const artifact of [...report.artifacts, ...report.candidates.map(item => item.artifact)]) {
    const path = resolveCanonicalArtifactPath(artifact.path);
