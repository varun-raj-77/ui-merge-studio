...
test('enables quick view only for configured stable product IDs', () => { render(<ProductGrid products={products} />); const targets = new Set<string>(quickViewTargetIds); expect(screen.queryAllByRole('button', { name: /^Quick view / })).toHaveLength(targets.size); for (const product of products) expect(Boolean(screen.queryByRole('button', { name: `Quick view ${product.name}` }))).toBe(targets.has(String(product.id))); });
