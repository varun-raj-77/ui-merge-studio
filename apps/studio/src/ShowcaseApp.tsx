...
const concepts = {
  navigation: { publicName: 'Focus Mode', title: feature('navigation').name, benefit: 'Collapse the navigation to create more workspace room without losing access to it.', version: 'Version A', accent: 'coral' },
  activity: { publicName: 'Activity Lens', title: feature('activity').name, benefit: 'Use All, Note, Status, and Email to isolate the activity you need.', version: 'Version B', accent: 'violet' }
} as const;
function recordedExclusion(id: ShowcaseFeatureId, path: string, symbol: string, label: string) {
  const evidence = feature(id).excludedFiles.find(item => item.path === path && item.symbol === symbol);
  if (!evidence) throw new Error(`Missing recorded exclusion evidence for ${path}#${symbol}.`);
  return { ...evidence, label };
}
const publicExclusions = {
  navigation: recordedExclusion('navigation', 'src/features/tickets/TicketPage.tsx', 'TicketPage', 'Operations Command Center heading'),
  activity: recordedExclusion('activity', 'src/utils/sortTickets.ts', 'sortTicketsNewestFirst', 'Newest-first sorting change')
} as const;
...
function Preview({ version }: { version: Version }) {
  ...
  const activeConcept = isA ? concepts.navigation : isB ? concepts.activity : null;
  const activeEvidence = isA ? feature('navigation') : isB ? feature('activity') : null;
  return <div className={`active-preview ${isA ? 'accent-a' : isB ? 'accent-b' : version === 'combined-result' ? 'accent-combined' : ''}`}>
    ...
    <div className="preview-stage"><iframe key={version} title={`${item.label} compiled Support Desk application`} src={artifactReplayUrl(version)} data-artifact-path={`${item.path}index.html`} loading="eager" sandbox="allow-scripts allow-same-origin" tabIndex={version === 'combined-result' ? 0 : -1} />{activeConcept && activeEvidence && <><div className={`behavior-callout ${isA ? 'callout-navigation' : 'callout-activity'}`}><span>Visible behavior</span><strong>{activeConcept.title}</strong><small>Mapped React boundary: {activeEvidence.selectedBoundary}</small></div><div className={`behavior-target ${isA ? 'target-navigation' : 'target-activity'}`} data-behavior-target={isA ? 'navigation' : 'activity'} aria-hidden="true" /></>}</div>
  </div>;
}
...
function IntegrationProof({ headingRef }: { headingRef: RefObject<HTMLHeadingElement | null> }) {
  return <section className="integration-proof" aria-labelledby="integration-title"><header><p className="overline">Recorded candidate replay</p><h2 id="integration-title" ref={headingRef} tabIndex={-1}>Selected source in. Unrelated edits out.</h2><p>Replaying the verified candidate-generation result.</p></header><div className="integration-grid"><div className="integration-tree"><div className="tree-root"><b>main</b><small>{showcaseManifest.repository.commonBaseCommit.slice(0, 8)}</small></div>{showcaseManifest.features.map(item => <div className={`tree-branch ${item.id}`} key={item.id}><span>{concepts[item.id].publicName}</span><strong>{item.sourceFile}</strong>{item.supportingFiles.slice(0, 4).map(file => <small key={file.path}>↳ {file.path}</small>)}</div>)}<div className="tree-result"><b>combined-result</b><small>{showcaseManifest.repository.candidateCommit.slice(0, 8)}</small></div></div><div className="integration-steps">{['Shared base confirmed','Selected source mapped','Required source set recorded','Unrelated changes excluded','Verified candidate created'].map((step, index) => <div key={step}><b>✓</b><span><small>0{index + 1}</small><strong>{step}</strong></span></div>)}</div></div>
    <div className="included-excluded"><article><header><span>Version A · Focus Mode</span></header><div className="included"><b>Included</b><strong>{feature('navigation').name} + {feature('navigation').supportingFiles.length} supporting files</strong></div><div className="excluded"><b>Excluded</b><strong>{publicExclusions.navigation.label}</strong></div></article><article><header><span>Version B · Activity Lens</span></header><div className="included"><b>Included</b><strong>{feature('activity').name} + {feature('activity').supportingFiles.length} supporting files</strong></div><div className="excluded"><b>Excluded</b><strong>{publicExclusions.activity.label}</strong><small>{publicExclusions.activity.path}</small></div></article></div>
  </section>;
}

function ResultProofStrip() {
  return <section className="result-proof-strip" aria-label="Combined result composition">
    <div><span>Included</span><ul><li><b>{feature('navigation').name}</b><small>Version A</small></li><li><b>{feature('activity').name}</b><small>Version B</small></li></ul></div>
    <div className="proof-excluded"><span>Excluded</span><ul><li>{publicExclusions.navigation.label}</li><li>{publicExclusions.activity.label}</li></ul></div>
  </section>;
}
...
function UnsafeChallenge({ state, update }: { state: LabState; update: (patch: Partial<LabState>) => void }) {
  ...
  return <section className="unsafe-panel"><header><span>Optional safety challenge · separately tested fixture</span><button onClick={() => update({ unsafeScenarioActive: false, refusalVisible: false })}>Close</button></header><div className="compatibility-summary"><small>Runtime-state compatibility</small><strong>These versions represent the selected ticket in different URL formats.</strong><p>UI Merge Studio checks whether their live state can stay synchronized safely before candidate work begins.</p></div>{!state.refusalVisible ? <button className="danger-action" onClick={() => update({ refusalVisible: true })}>Check compatibility</button> : <div className="refusal" role="alert"><span>Preview synchronization refused</span><h2>No candidate was attempted or created.</h2><p>{refusalEvidence.publicReason}</p><dl><div><dt>Safe next step</dt><dd>{refusalEvidence.next}</dd></div></dl></div>}<details className="contract-details"><summary>Technical details</summary><div className="contract-comparison"><div><small>Version A adapter</small><strong>{refusalEvidence.leftContract}</strong></div><b>≠</b><div><small>Incompatible route adapter</small><strong>{refusalEvidence.rightContract}</strong></div></div><dl><div><dt>Exact compatibility result</dt><dd>{refusalEvidence.reason}</dd></div><div><dt>Affected route</dt><dd><code>{refusalEvidence.route}</code></dd></div><div><dt>Fixture-backed provenance</dt><dd><code>{refusalEvidence.evidence}</code></dd></div></dl></details></section>;
}
