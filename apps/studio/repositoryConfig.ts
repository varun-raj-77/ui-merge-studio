...
export function loadRepositoryConfiguration(workspaceRoot: string, environment: Environment = process.env) {
  return {
    repositoryPath: environment.UI_MERGE_REPOSITORY_PATH ?? environment.UI_MERGE_FIXTURE_PATH ?? resolve(workspaceRoot, 'fixtures/generated/support-dashboard'),
    baseRef: environment.UI_MERGE_BASE_REF ?? 'main',
    previewPath: environment.UI_MERGE_PREVIEW_ROUTE ?? '/tickets',
