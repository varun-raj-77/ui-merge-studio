...
Current preview execution is bounded to local React + TypeScript + Vite repositories with a declared `dev` script. npm command resolution, real installation, and real Vite launch are covered by integration tests. pnpm and yarn command/argument resolution is implemented, but real installation and Vite launch remain unverified; default candidate verification also remains npm-specific. Preview installs run the package manager's ordinary `install` command inside the detached worktree, may mutate that worktree, use the network, and execute repository lifecycle scripts as normal local code. This does not mutate the original checkout, but it is not an immutable or fully reproducible install. Port selection has a bounded check-then-launch race; Vite receives `--strictPort`, so a collision fails instead of silently changing ports.

Region projection supports only a uniquely identified added JSX component that is already a direct child of one structurally compatible parent, with a unique unchanged sibling anchor or an empty parent. Expression-enclosing child replacement and any failed region projection are refused; a region operation never falls back to whole-declaration replacement. Independently declaration-owned changes may still use declaration replacement, but integration-boundary expansion alone is not ownership proof. Next.js, arbitrary monorepos, cloud repository execution, GitHub authentication, collaboration, and guaranteed integration of arbitrary branch combinations are not supported.
