...
UI Merge Studio is a local React/Vite product proof for comparing two running UI versions, choosing one visible feature from each, and creating one verified combined branch. A dedicated first screen explains the fictional Sample Support Dashboard, the branch relationship, the example task, and the safety promises before any preview starts. The default experience is Guided Mode: **Compare → Select → Combine → Verify**.

Run `npm run dev`, open `http://127.0.0.1:4310`, and choose **Start guided comparison**. The responsive comparison supports both, Version A focus, and Version B focus layouts without horizontal preview dragging. Guided copy uses human branch labels; raw refs, source identities, Git/AST evidence, operation timings, and deterministic artifacts remain available under **Technical details**.

The controlled default compares **Collapsible Sidebar Variant** from the Navigation experiment with **Activity Filters Variant** from the Activity-filter experiment. Guided selection accepts the focused example targets and stops broader targets before candidate creation. Those rules are presentation safety for this controlled demonstration; the engine continues to operate on validated branch, commit, session, AST, and Git evidence.
