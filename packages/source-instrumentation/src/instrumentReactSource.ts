...
export interface InstrumentationOptions { repositoryRoot: string; branch: string }
...
function addMetadata(node: t.JSXElement, metadata: StaticBoundaryMetadata) {
  ...
  node.openingElement.attributes.push(t.jsxAttribute(t.jsxIdentifier(sourceAttribute), t.jsxExpressionContainer(t.stringLiteral(JSON.stringify(metadata)))));
}
function instrumentExpression(expression: t.Expression | t.JSXElement | t.JSXFragment, metadata: StaticBoundaryMetadata): number {
  if (t.isJSXElement(expression)) { if (isHostElement(expression)) { addMetadata(expression, metadata); return 1; } return 0; }
  if (t.isJSXFragment(expression)) {
    let count = 0;
    const partial = { ...metadata, confidence: 'partial' as const };
    for (const child of expression.children) {
      if (t.isJSXElement(child) && isHostElement(child)) { addMetadata(child, partial); count += 1; }
      else if (t.isJSXExpressionContainer(child) && t.isExpression(child.expression)) count += instrumentExpression(child.expression, partial);
    }
    return count;
  }
  if (t.isConditionalExpression(expression)) return instrumentExpression(expression.consequent, metadata) + instrumentExpression(expression.alternate, metadata);
  if (t.isLogicalExpression(expression) && t.isExpression(expression.right)) return instrumentExpression(expression.right, metadata);
  return 0;
}
...
function instrumentFunction(path: NodePath<t.FunctionDeclaration | t.FunctionExpression | t.ArrowFunctionExpression>, metadata: StaticBoundaryMetadata) {
  if (t.isArrowFunctionExpression(path.node) && !t.isBlockStatement(path.node.body)) return instrumentExpression(path.node.body, metadata);
  let count = 0;
  path.traverse({ ReturnStatement(returnPath) { if (returnPath.getFunctionParent() !== path || !returnPath.node.argument) return; count += instrumentExpression(returnPath.node.argument, metadata); } });
  return count;
}
function pathFor(repositoryRoot: string, id: string) { return relative(resolve(repositoryRoot), resolve(id)).split(sep).join('/'); }
function stableId(relativePath: string, line: number, column: number, name: string | null) { return createHash('sha256').update(`${relativePath}:${line}:${column}:${name ?? 'anonymous'}`).digest('hex').slice(0, 16); }
...
export function instrumentReactSource(code: string, id: string, options: InstrumentationOptions) {
  ...
  const makeMetadata = (node: t.Node, componentName: string | null, exportName: string | null): StaticBoundaryMetadata => {
    ...
    return { boundaryId: stableId(repositoryRelativePath, line, column, componentName), repositoryRelativePath, line, column, componentName, exportName, branch: options.branch, confidence: 'exact' };
  };
  babelTraverse(ast, {
    FunctionDeclaration(path) {
      ...
      boundaries += instrumentFunction(path, makeMetadata(path.node, name, exportNameFor(path)));
    },
    VariableDeclarator(path) {
      ...
      boundaries += instrumentFunction(path.get('init') as NodePath<t.ArrowFunctionExpression | t.FunctionExpression>, makeMetadata(path.node, path.node.id.name, exportNameFor(path)));
    },
    ExportDefaultDeclaration(path) {
      const declaration = path.node.declaration;
      if (!t.isArrowFunctionExpression(declaration) && !t.isFunctionExpression(declaration)) return;
      boundaries += instrumentFunction(path.get('declaration') as NodePath<t.ArrowFunctionExpression | t.FunctionExpression>, makeMetadata(declaration, t.isFunctionExpression(declaration) ? declaration.id?.name ?? null : null, 'default'));
    }
  });
  if (!boundaries) return null;
  const output = babelGenerate(ast, { sourceMaps: true, sourceFileName: repositoryRelativePath }, code);
  return { code: output.code, map: output.map, boundaries };
}
