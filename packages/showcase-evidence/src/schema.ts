export const showcaseSchemaVersion = 1 as const;
export type ArtifactId = 'baseline' | 'branch-a' | 'branch-b' | 'combined-result';
export type FeatureId = 'navigation' | 'activity';
...
export function validatePublicShowcaseReport(value: unknown): PublicShowcaseReport {
  ...
  if (report.schemaVersion !== showcaseSchemaVersion) fail('unsupported schema version');
  if (!report.runId || !report.generatedAt || report.fixture !== 'fixtures/generated/support-dashboard') fail('missing run identity');
  ...
  if (report.selectedFeatureIds.join(',') !== 'navigation,activity' || report.features?.length !== 2) fail('both controlled selections are required');
  for (const feature of report.features) {
    if (!feature.sourceFile?.startsWith('src/') || !feature.selectedBoundary || !feature.supportingFiles?.length || !feature.excludedFiles?.length) fail(`feature ${feature.id} is incomplete`);
  }
  ...
  for (const href of Object.values(report.links ?? {})) {
    if (!href.startsWith('https://github.com/varun-raj-77/ui-merge-studio')) fail(`invalid public link: ${href}`);
  }
  return report;
}
