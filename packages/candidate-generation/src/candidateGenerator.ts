...
export class CandidateGenerator {
  ...
  private generationId(request:CandidateGenerationRequest){return createHash('sha256').update(JSON.stringify({version:candidateGenerationVersion,repositoryRoot:resolve(request.repositoryRoot),repositoryId:request.repositoryId,baseRef:request.baseRef,expectedBaseCommit:request.expectedBaseCommit,commonBaseRef:request.commonBaseRef??request.baseRef,expectedCommonBaseCommit:request.expectedCommonBaseCommit??request.expectedBaseCommit,candidateBranch:request.candidateBranch,analyzerSchemaVersion:request.analyzerSchemaVersion,sliceIds:stable(request.artifacts.map(item=>item.analysisId),value=>value),sourceConfigurations:stable(request.sourceConfigurations ?? [],item=>`${item.sliceId}:${item.path}:${item.declaration}`)})).digest('hex').slice(0,16);}
