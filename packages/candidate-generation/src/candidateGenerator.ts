...
export class CandidateGenerator {
  ...
  private async validate(request: CandidateGenerationRequest) {
    ...
    for (const artifact of request.artifacts) {
      ...
      if (slice.repository.baseRef!==request.baseRef || slice.repository.mergeBaseCommit!==request.expectedBaseCommit) reject(`Slice ${id} does not use the expected base commit.`,'<slice>',id);
  ...
  private async buildOperations(request: CandidateGenerationRequest, operations: CandidateOperation[], unresolved: CandidateUnresolved[]) {
    ...
    for(const artifact of stable(request.artifacts,item=>item.analysisId)) {
      ...
      for(const [path,changes] of stable([...includedByPath.entries()],item=>item[0])) {
        ...
        for(const change of changes) if(change.symbol){const record=sourceModule.declarations.find(item=>item.name===change.symbol!.name); if(!record){this.unresolved(unresolved,path,artifact.analysisId,`Source declaration ${change.symbol.name} is missing.`);continue;} [...record.dependencies,...record.jsxReferences].forEach(item=>dependencies.add(item)); const sourceSnippet=sourceText.slice(record.start,record.end); const baseRecord=baseModule.declarations.find(item=>item.name===record.name); const baseSnippet=baseRecord?baseText.slice(baseRecord.start,baseRecord.end):null; const kind=baseRecord?(change.category==='integration'?'replace-jsx-region':'replace-declaration'):'insert-declaration'; operations.push(this.operation(kind,artifact,path,record.name,{startLine:record.startLine,endLine:record.endLine},sourceSnippet,textHash(sourceSnippet),change.evidenceEdgeIds,`${baseRecord?'Replace':'Insert'} declaration ${record.name} by AST identity from the validated source branch.`,undefined,undefined,textHash(baseText),baseSnippet?textHash(baseSnippet):null,baseRecord?{startLine:baseRecord.startLine,endLine:baseRecord.endLine}:null));}
