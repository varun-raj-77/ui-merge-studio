...
test('generates and verifies the Product Catalogue candidate, then repeats idempotently',async()=>{
  ...
  const request={repositoryRoot:fixture,baseRef:'main',expectedBaseCommit:base,candidateBranch:'combined-result',artifacts:await artifacts(),analyzerSchemaVersion:2 as const};
