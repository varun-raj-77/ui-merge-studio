...
test('returns a deterministic partial slice for a dynamic unresolved dependency and cycles terminate', async () => {
  ...
  expect(first.status).toBe('partial'); expect(first.boundary.status).toBe('expanded-to-integration-boundary'); expect(first.unresolvedDependencies.some(item => item.reason.includes('Dynamic import'))).toBe(true); expect(second).toEqual(first);
});
