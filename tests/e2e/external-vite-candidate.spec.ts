...
test('creates, verifies, relaunches, and deterministically recognizes the external candidate', async ({ page, request }) => {
  ...
  await expect(card(page, 'left').locator('.attached-selection')).toBeVisible({ timeout: 90_000 });
  ...
  await expect(card(page, 'right').locator('.attached-selection')).toBeVisible({ timeout: 90_000 });
  ...
  await expect(dependencySections.nth(0)).toContainText('validationWorkspace');
  await expect(dependencySections.nth(0)).toContainText('validationWorkspaceConfig');
  await expect(dependencySections.nth(0)).toContainText('src/components/layout/headerbar.tsx');
  await expect(dependencySections.nth(1)).toContainText('revenueOutlook');
  await expect(dependencySections.nth(1)).toContainText('revenueOutlookConfig');
  ...
  await expect(page.getByRole('button', { name: 'Open combined preview' })).toBeVisible();
  ...
  await expect(page.getByRole('heading', { name: candidateBranch })).toBeVisible();
