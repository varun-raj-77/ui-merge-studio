...
test('resolves exact instance selections to a distinct generated candidate artifact', async ({ page }) => {
  await page.goto(url);
...
test('deselection updates the exact generated artifact while staying in result view', async ({ page }) => {
  await page.goto(url);
  const versionB = page.frameLocator('iframe[title="Version B live application"]');
  ...
  await expect(page.getByRole('heading', { name: 'Built from 1 selection' })).toBeVisible();
});
