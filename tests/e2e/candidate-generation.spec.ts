...
test('creates, verifies, repeats idempotently, and opens the real two-feature candidate', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Combined branch ready' })).toBeVisible({ timeout: 300_000 });
  ...
  await page.getByRole('button', { name: 'Combined branch ready' }).click();
  await expect(page.getByRole('button', { name: 'Open verified result' })).toBeVisible({ timeout: 300_000 });
