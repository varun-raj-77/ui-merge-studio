...
async function prepareResolvedFeatures(page: Page, expectReady = true) {
  ...
  await expect(page.locator('.workspace-status')).toHaveText('Both versions are ready to compare', { timeout: 180_000 });
...
test('creates, verifies, repeats idempotently, and opens the real two-feature candidate', async ({ page }) => {
  ...
  await page.screenshot({ path: 'docs/evidence/prompt-006c/result-1440x900.png', fullPage: false });
