...
function artifact(selection: SourceIdentity, status: SliceStatus, analyzed: string): FeatureSliceArtifact {
  const testFileSlices = selection.componentName === 'ActivityFilters' ? [{ path:'src/test/inspector.test.tsx', mode:'test-units' as const, includedUnits:[{ id:'test-1',kind:'test' as const,title:'filters activity',region:{startLine:3,endLine:3},changedHunks:[{startLine:3,endLine:3}],reason:'Evidence-connected UI contract.',evidenceEdgeIds:['test-edge'],confidence:'exact' as const,enclosingDescribeIds:[] }], excludedUnits:[{ id:'test-2',kind:'test' as const,title:'sorts tickets',region:{startLine:4,endLine:4},changedHunks:[{startLine:4,endLine:4}],reason:'No supported dependency edge.',evidenceEdgeIds:['excluded-edge'],confidence:'exact' as const,enclosingDescribeIds:[],proof:'proven' as const }], requiredImports:[{source:'./renderApp',local:'renderApp',imported:'renderApp',kind:'value',reason:'Required by included test.'}], excludedImports:[{source:'./sort',local:'sortTickets',imported:'sortTickets',kind:'value',reason:'Used only by excluded test.'}], requiredSupportDeclarations:[], unresolvedDependencies:[], evidenceEdgeIds:['test-edge'] }] : [];
  return { analysisId: `${selection.previewId === 'left' ? 'a' : 'b'}`.repeat(16), relativePath: '.ums/analysis/result/feature-slice.json', slice: { version: 2, repository: { baseRef: 'main', branchRef: selection.branch, mergeBaseCommit: '1111111111111111111111111111111111111111', branchCommit: '2222222222222222222222222222222222222222' }, selection, status, boundary: { original: selection.componentName!, analyzed, status: analyzed === selection.componentName ? 'selected-boundary-sufficient' : 'expanded-to-integration-boundary', reason: analyzed === selection.componentName ? 'Selected boundary is sufficient.' : `Expanded to ${analyzed} through changed integration edges.` }, changedFiles: [], includedChanges: [{ path: selection.repositoryRelativePath, category: 'selected-definition', symbol: { name: selection.componentName!, kind: 'component', region: { startLine: 4, endLine: 8 } }, branchChangeId: `change-${selection.previewId}`, wholeFile: false, reason: 'Validated visual selection seed.', evidenceEdgeIds: ['edge-1'], confidence: 'exact' }], excludedChanges: [{ path: 'src/Unrelated.tsx', symbol: { name: 'Unrelated', kind: 'component', region: { startLine: 1, endLine: 2 } }, branchChangeId: `excluded-${selection.previewId}`, classification: 'proven-unrelated', proof: 'proven', reason: 'Outside the supported graph.' }], unresolvedDependencies: status === 'partial' ? [{ path: selection.repositoryRelativePath, symbol: null, reason: 'Dynamic import', edge: 'unresolved-static-analysis', manualNextStep: 'Choose a supported ancestor.', ancestorBoundaryMayHelp: true }] : [], testFileSlices, evidence: [] } };
}
...
test('starts analysis for the correct preview and renders independent resolved and partial evidence', async () => {
  ...
  expect(within(rightCard).getByText((_, element) => element?.tagName === 'P' && element.textContent === 'Mode: test-units')).toBeVisible(); expect(within(rightCard).getByText('filters activity')).toBeVisible(); expect(within(rightCard).getByText('sorts tickets')).toBeVisible();
  expect(within(rightCard).getByText('renderApp ← ./renderApp#renderApp')).toBeVisible(); expect(within(rightCard).getByText('sortTickets ← ./sort#sortTickets')).toBeVisible();
  expect(within(leftCard).getByRole('heading', { name: 'Feature slice · resolved' })).toBeVisible();
});

test('shows an explicit partial shared-setup warning inside a test-file slice', () => {
  const preview = session('right','branch-inspector'); const value = artifact(sourceIdentity(preview,'ActivityFilters'),'partial','TicketInspector'); const file = value.slice.testFileSlices[0]; file.mode='partial'; file.unresolvedDependencies=[{ path:file.path,symbol:'beforeEach',reason:'Top-level beforeEach contains inseparable setup.',edge:'ambiguous-shared-setup',manualNextStep:'Split setup.',ancestorBoundaryMayHelp:false }];
  render(<SlicePanel artifact={value} status="partial" error={null} />); expect(screen.getByText('Partial test slicing:')).toBeVisible(); expect(screen.getByText(/inseparable setup/)).toBeVisible();
});
