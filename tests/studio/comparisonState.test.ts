...
const session = (previewId: 'left' | 'right', branch: string, generation = 1) => ({ previewId, branch, generation, sessionId: `${previewId}-${generation}`, protocolVersion: bridgeVersion, repositoryPath: '/tmp/repository', commit: `${branch}-commit`, branchCommit: `${branch}-commit`, packageManager: 'npm' as const, url: `http://${previewId}/catalogue`, origin: `http://${previewId}`, port: previewId === 'left' ? 5001 : 5002, processId: previewId === 'left' ? 5001 : 5002, worktreePath: `/tmp/${previewId}`, status: 'running' as const, failure: null });
