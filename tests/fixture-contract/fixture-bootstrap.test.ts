...
test('generates deterministic Sample Support Desk branding on every branch', () => { for (const branch of ['main', 'branch-sidebar', 'branch-inspector', 'branch-incompatible-route']) { const source = git(generated, ['show', `${branch}:src/features/navigation/AppSidebar.tsx`]); expect(source).toContain('Sample Support Desk'); expect(source).toContain('Demo application · Fake ticket data'); expect(source).not.toContain('Beacon Ops'); } });
...
test('rejects tags and an unverified combined result', () => { const tagged = clone(); git(tagged, ['tag', 'sidebar-feature']); expectReject(tagged, 'tags are forbidden'); const candidate = clone(); git(candidate, ['branch', 'combined-result']); expectReject(candidate, 'combined-result must be exactly one verified generated commit ahead'); });
