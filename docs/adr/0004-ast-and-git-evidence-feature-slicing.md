...
Include changed declarations at symbol/region granularity when supported. Include CSS, assets, module-level style registration, and statically related tests as whole files when finer separation is not safe, and mark them conservative. Classify every remaining branch item as affirmatively proven unrelated only when the supported graph is complete for that decision; otherwise label it unreached/unsupported. Unsupported dynamic edges, deleted/binary dependencies, stale evidence, and unresolved boundaries produce `partial` or `refused` output.
...
- Whole-file stylesheet and test inclusion is deliberately conservative; the inspector test file remains broader than the production feature slice.
